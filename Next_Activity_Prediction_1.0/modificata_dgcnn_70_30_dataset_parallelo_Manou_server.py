...
#     target_std = {}
#     i = 0
#     input = open("./dataset/" + chosen_data + "/target_std.txt", "r")
...
#     target_par = {}
#     i = 0
#     input = open("./dataset/" + chosen_data + "/target_par.txt", "r")
#     for lines in input.readlines():
#         lines = lines[:-1]
#         target_par[lines] = i
#         i = i + 1
...
class SortPool(torch.nn.Module):
    ...
    def __init__(self, dataset, num_layers, hidden):
        ...
        #MOD MR
        self.lin2 = Linear(hidden, num_class)  # Per classificazione multiclasse
    ...
    def forward(self, data):
        ...
        x = global_sort_pool(x, batch, self.k)  # layer di pooling
...
from creazione_dataset_parallelo_Manou_server import TraceDataset as TD
...
terget_par_path = PATH+'/target_par.txt'
num_class = 0
with open(terget_par_path, 'r') as file:
    for row in file:
        row = row.strip()
        num_class += 1
...
def plot_confusion_matrix(y_true, y_pred, normalize=False, title='Confusion matrix', cmap=plt.cm.Blues):
    ...
    plt.figure(figsize=(15, 12))  # Aumenta le dimensioni della figura
    plt.imshow(cm, interpolation='nearest', cmap=cmap)
    plt.title(title)
    plt.colorbar()
    tick_marks = np.arange(cm.shape[0])
    plt.xticks(tick_marks, rotation=90, fontsize=8)  # Ruota e riduci la dimensione del font
    plt.yticks(tick_marks, fontsize=8)  # Riduci la dimensione del font
    ...
    # Aggiungi le metriche di valutazione alla parte inferiore
    plt.text(cm.shape[1], cm.shape[0], f'Accuracy: {acc:.4f}\nF1 Score: {f1:.4f}', 
...
with open('./Output/dataset/target_par.txt', 'r') as file:
...
for epoch in range(start_epoch, start_epoch + delta_epoch): #0-100  # ciclo delle epoche di addestramento
    ...
    plot_confusion_matrix(y_true_test, y_pred_test, title="Confusion matrix TEST epoch : " + str(epoch))
    ...
    # Plot della matrice di confusione
    plt.figure(figsize=(15, 15))
    plot_confusion_matrix(y_true_train, y_pred_train, title="Confusion matrix TEST epoch : " + str(epoch))
    ...
    if os.path.isdir(args.checkpoint_dir+"/Immagini/cm_epoch"):
        ...
        if test_loss < best_loss_test or epoch == 0:  # condizione per decidere la miglior loss di test
            ...
            plt.figure(figsize=(15, 15))
            plot_confusion_matrix(y_true_test, y_pred_test, title="Confusion matrix BEST TEST epoch : " + str(epoch))
        ...
        if train_loss < best_loss_train or epoch == 0:  # condizione per decidere la miglior loss di test
            ...
            plt.figure(figsize=(15, 15))
            plot_confusion_matrix(y_true_train, y_pred_train, title="Confusion matrix BEST TRAIN epoch : " + str(epoch))
